...
from icon_registration.losses import BlurredSSD
from utils.loss import VectorFieldLoss

TransMorphLoss = namedtuple(
    "TransMorphLoss",
    ["all_loss", "similarity_loss", "regularizer_loss"],
)
...
class SSIM_loss(torch.nn.Module):
    def __init__(self, if_MS=True, win_size=11, size_average=True):
        super(SSIM_loss, self).__init__()
        if if_MS:
            self.SSIM = MS_SSIM(win_size=win_size, data_range=255, size_average=True, channel=1)
        else:
            self.SSIM = SSIM(data_range=255, size_average=size_average, channel=1)
...
def compute_bending_energy_loss(phi_AB_vectorfield):
    ...
    if len(phi_AB_vectorfield.shape)-1 == 3:
        ...
        dzdz = (phi_AB_vectorfield[:, :, :, :, 2:] 
            - 2*phi_AB_vectorfield[:, :, :, :, 1:-1]
            + phi_AB_vectorfield[:, :, :, :, :-2]) / spacing[2]**2
        dxdy = (phi_AB_vectorfield[:, :, 2:, 2:] 
            + phi_AB_vectorfield[:, :, :-2, :-2] 
            - phi_AB_vectorfield[:, :, 2:, :-2]
            - phi_AB_vectorfield[:, :, :-2, 2:]) / (4.0*spacing[0]*spacing[1])
        dydz = (phi_AB_vectorfield[:, :, :, 2:, 2:] 
            + phi_AB_vectorfield[:, :, :, :-2, :-2] 
            - phi_AB_vectorfield[:, :, :, 2:, :-2]
            - phi_AB_vectorfield[:, :, :, :-2, 2:]) / (4.0*spacing[1]*spacing[2])
        dxdz = (phi_AB_vectorfield[:, :, 2:, :, 2:] 
            + phi_AB_vectorfield[:, :, :-2, :, :-2] 
            - phi_AB_vectorfield[:, :, 2:, :, :-2]
            - phi_AB_vectorfield[:, :, :-2, :, 2:]) / (4.0*spacing[0]*spacing[2])
...
class transmorph_loss(torch.nn.Module):
    def __init__(self,weight = 1.0):
        super().__init__()
        # self.similarity_loss = SSIM_loss(False)
        self.similarity_loss = BlurredSSD(sigma=4)
        self.weight = weight
        # if regularizer == "Grad3d":
        # self.regualrizer = Grad(penalty='l2')
        self.regualrizer = compute_bending_energy_loss
    def forward(self, y_pred_ab, y_true_ab, y_pred_field_ab, y_pred_ba, y_true_ba, y_pred_field_ba):
        sim_loss = (self.similarity_loss(y_pred_ab, y_true_ab)) + (self.similarity_loss(y_pred_ba, y_true_ba))
        # reg_loss = self.regualrizer(y_pred_field_ab) + self.regualrizer(y_pred_field_ba)
        VFloss = VectorFieldLoss().to(y_pred_ab.device)
        VFloss.assign_identity_map(y_pred_field_ba.shape)
        reg_loss = VFloss.icon(y_pred_field_ab, y_pred_field_ba)
        
        total_loss = sim_loss + self.weight * reg_loss
        return TransMorphLoss(
            total_loss,
            sim_loss,
            reg_loss,
        )